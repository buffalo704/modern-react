...
export const Button = ({children, primary, secondary, success, warning, danger, outline, rounded, loading, ...rest}:PropsWithChildren<ButtonProps>) => {
  ...
  const buttonChildren = loading ? <GoSync className="animate-spin"/> : children;
  return <button {...rest} disabled={loading} className={classes}>{buttonChildren}</button>;
}
